...
from faceswapper import process_manager
from faceswapper.thread_helper import thread_lock, thread_semaphore
from faceswapper.typing import ModelSet, AudioChunk, Audio
from faceswapper.execution import apply_execution_provider_options
from faceswapper.filesystem import resolve_relative_path, is_file
...
MODELS : ModelSet =\
{
	'voice_extractor':
	{
		'url': 'https://github.com/faceswapper/faceswapper-assets/releases/download/models/voice_extractor.onnx',
...
def get_voice_extractor() -> Any:
	...
	with thread_lock():
		...
		if VOICE_EXTRACTOR is None:
			...
			VOICE_EXTRACTOR = onnxruntime.InferenceSession(model_path, providers = apply_execution_provider_options(faceswapper.globals.execution_device_id, faceswapper.globals.execution_providers))
...
def pre_check() -> bool:
	...
	if not faceswapper.globals.skip_download:
