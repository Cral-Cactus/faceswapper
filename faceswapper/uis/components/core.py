...
from faceswapper import metadata, wording
from faceswapper.typing import Frame
from faceswapper.uis.typing import Component, ComponentName
...
def launch() -> None:
	with gradio.Blocks(theme = get_theme(), title = metadata.get('name') + ' ' + metadata.get('version')) as ui:
		for ui_layout in faceswapper.globals.ui_layouts:
...
def load_ui_layout_module(ui_layout : str) -> Any:
	try:
		ui_layout_module = importlib.import_module('faceswapper.uis.layouts.' + ui_layout)
