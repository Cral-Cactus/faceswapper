...
from faceswapper.typing import OutputVideoEncoder
...
def render() -> None:
	...
	with gradio.Box():
		OUTPUT_VIDEO_ENCODER_DROPDOWN = gradio.Dropdown(
			...
			choices = faceswapper.choices.output_video_encoder,
			value = faceswapper.globals.output_video_encoder
		)
		OUTPUT_VIDEO_QUALITY_SLIDER = gradio.Slider(
			label = wording.get('output_video_quality_slider_label'),
			value = faceswapper.globals.output_video_quality,
			step = 1
		)
...
def update_output_video_encoder(output_video_encoder: OutputVideoEncoder) -> Update:
	faceswapper.globals.output_video_encoder = output_video_encoder
	return gradio.update(value = output_video_encoder)


def update_output_video_quality(output_video_quality : int) -> Update:
	faceswapper.globals.output_video_quality = output_video_quality
