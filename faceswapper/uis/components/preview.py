...
from faceswapper.capturer import get_video_frame, get_video_frame_total
from faceswapper.face_analyser import get_one_face
from faceswapper.face_reference import get_face_reference, set_face_reference
from faceswapper.predictor import predict_frame
from faceswapper.processors.frame.core import load_frame_processor_module
from faceswapper.typing import Frame
from faceswapper.uis import core as ui
from faceswapper.uis.typing import ComponentName, Update
from faceswapper.utilities import is_video, is_image
...
def render() -> None:
	...
	with gradio.Box():
		...
		if is_image(faceswapper.globals.target_path):
			target_frame = cv2.imread(faceswapper.globals.target_path)
		...
		if is_video(faceswapper.globals.target_path):
			temp_frame = get_video_frame(faceswapper.globals.target_path, faceswapper.globals.reference_frame_number)
			preview_frame = extract_preview_frame(temp_frame)
			preview_image_args['value'] = ui.normalize_frame(preview_frame)
			preview_image_args['visible'] = True
			preview_frame_slider_args['value'] = faceswapper.globals.reference_frame_number
			preview_frame_slider_args['maximum'] = get_video_frame_total(faceswapper.globals.target_path)
...
def update(frame_number : int = 0) -> Tuple[Update, Update]:
	...
	if is_video(faceswapper.globals.target_path):
		faceswapper.globals.reference_frame_number = frame_number
...
def extract_preview_frame(temp_frame : Frame) -> Frame:
	...
	source_face = get_one_face(cv2.imread(faceswapper.globals.source_path)) if faceswapper.globals.source_path else None
	...
	for frame_processor in faceswapper.globals.frame_processors:
