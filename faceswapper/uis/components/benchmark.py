...
from faceswapper.utilities import normalize_output_path
...
def update(benchmark_cycles : int) -> Update:
	faceswapper.globals.source_path = '.assets/examples/source.jpg'
	target_paths =\
	[
...
def benchmark(target_path : str, cycles : int) -> List[Any]:
	...
	for i in range(cycles + 1):
		faceswapper.globals.target_path = target_path
		faceswapper.globals.output_path = normalize_output_path(faceswapper.globals.source_path, faceswapper.globals.target_path, tempfile.gettempdir())
	...
	return\
	[
		faceswapper.globals.target_path,
		cycles,
