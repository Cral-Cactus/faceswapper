...
from faceswapper.face_analyser import clear_face_analyser
from faceswapper.processors.frame.core import clear_frame_processors_modules
from faceswapper.uis.typing import Update
from faceswapper.utilities import encode_execution_providers, decode_execution_providers
...
def render() -> None:
	...
	with gradio.Box():
		EXECUTION_PROVIDERS_CHECKBOX_GROUP = gradio.CheckboxGroup(
			...
			value = encode_execution_providers(faceswapper.globals.execution_providers)
		)
		EXECUTION_THREAD_COUNT_SLIDER = gradio.Slider(
			label = wording.get('execution_thread_count_slider_label'),
			value = faceswapper.globals.execution_thread_count,
			step = 1,
			minimum = 1,
			maximum = 128
		)
		EXECUTION_QUEUE_COUNT_SLIDER = gradio.Slider(
			...
			value = faceswapper.globals.execution_queue_count,
			step = 1,
			minimum = 1,
			maximum = 16
		)
