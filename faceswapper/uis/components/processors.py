...
from faceswapper.processors.frame.core import load_frame_processor_module, clear_frame_processors_modules
from faceswapper.uis import core as ui
from faceswapper.uis.typing import Update
from faceswapper.utilities import list_module_names
...
def render() -> None:
	...
	with gradio.Box():
		FRAME_PROCESSORS_CHECKBOX_GROUP = gradio.CheckboxGroup(
			...
			choices = sort_frame_processors(faceswapper.globals.frame_processors),
			value = faceswapper.globals.frame_processors
		)
...
def update_frame_processors(frame_processors : List[str]) -> Update:
	...
	faceswapper.globals.frame_processors = frame_processors
...
def sort_frame_processors(frame_processors : List[str]) -> list[str]:
	frame_processors_names = list_module_names('faceswapper/processors/frame/modules')
