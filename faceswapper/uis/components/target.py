...
def render() -> None:
	...
	with gradio.Box():
		is_target_image = is_image(faceswapper.globals.target_path)
		is_target_video = is_video(faceswapper.globals.target_path)
		TARGET_FILE = gradio.File(
			...
			value = faceswapper.globals.target_path if is_target_image or is_target_video else None
		)
...
def update(file : IO[Any]) -> Tuple[Update, Update]:
	...
	if file and is_image(file.name):
		faceswapper.globals.target_path = file.name
	...
	faceswapper.globals.target_path = None
