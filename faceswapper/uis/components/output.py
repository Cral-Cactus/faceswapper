...
from faceswapper.core import conditional_process
from faceswapper.uis.typing import Update
from faceswapper.utilities import is_image, is_video, normalize_output_path
...
def update() -> Tuple[Update, Update]:
	faceswapper.globals.output_path = normalize_output_path(faceswapper.globals.source_path, faceswapper.globals.target_path, '.')
	if faceswapper.globals.output_path:
		conditional_process()
		if is_image(faceswapper.globals.output_path):
			return gradio.update(value = faceswapper.globals.output_path, visible = True), gradio.update(value = None, visible = False)
		if is_video(faceswapper.globals.output_path):
			return gradio.update(value = None, visible = False), gradio.update(value = faceswapper.globals.output_path, visible = True)
